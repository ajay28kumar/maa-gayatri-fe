...
const DataForm = ({
    ...
    const disabledStatus = !(category.length>0 && brand.length>0 && other.length>0 && !isNaN(rate) && unit.length>0);
    const stockData = {
        category,
        brand,
        others: other,
    ...
    return (
        <div style={{display: "flex", flexDirection: "column", alignItems: "center", maxWidth: 600, minWidth: 280}}>
                <TextField
                style={textStyle}
                variant="outlined"
                label="Category"
                ...
                />
                <Button variant="contained" disabled={disabledStatus} onClick={handleSubmit}>Add Data</Button>
        </div>
    )
};
